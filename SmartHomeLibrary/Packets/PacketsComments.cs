...
namespace SmartHomeTool.SmartHomeLibrary
{
	...
	{
		public static List<PacketDecodeAndCommentItem> PacketDecodeAndCommentItems = new List<PacketDecodeAndCommentItem>();
		...
		public static string DecodeFrameAndGetComment(byte[] data, out bool error)
		{
			...
			if (!Packets.FindFrameAndDecodePacketInBuffer(data, data.Length, out _, out _, out _,
					out dataOut, out frameCrc32, out calculatedCrc32, out isAnswer))
			{
				error = true;
		...
		public static string GetCommentToErrorFrame(ushort frameCrc16, ushort calculatedCrc16)
		{
			if (frameCrc16 != calculatedCrc16)
				return String.Format("Error: Frame CRC32={0:x8} != calculated CRC32={1:x8}.",
						frameCrc16, calculatedCrc16);
			else
				return "Error: No packet found.";
		}
		...
		static string ReplaceCommentPattern(string s, byte[] data)
		{
			...
			while (i < s.Length)
			{
				if (s[i++] == '[')
				{
					...
					if (j < s.Length)
					{
						...
						string u = s.Substring(i, j - i);
						string[] uu = u.Split(':');
						uint byteNr;
						if (uu.Length >= 2 && uint.TryParse(uu[0], out byteNr) && byteNr < data.Length)
						{
							if (uu.Length == 2)
							{
								byte d = data[byteNr];
								...
								if (uu[1] == "dec")
									t += d;
								else if (uu[1].Length == 4 && uu[1].Substring(0, 3) == "dec" &&
										int.TryParse(uu[1][3].ToString(), out digits))
									t += d.ToString("d" + digits);
								else if (uu[1] == "hex")
									t += d.ToString("x2");
								else if (uu[1].Length == 4 && uu[1].Substring(0, 3) == "hex" &&
										int.TryParse(uu[1][3].ToString(), out digits))
							...
							else if (uu.Length == 3)
							{
								...
								{
									int a;
									if (int.TryParse(uu[2], out a) && a >= 0 && a < 8)
								...
								else
								{
									...
									int a1, a2;
									if (uuu.Length == 2 && int.TryParse(uuu[0], out a1) && int.TryParse(uuu[1], out a2) &&
							...
							{
								byte mask, equal;
								if (byte.TryParse(uu[2], NumberStyles.HexNumber, CultureInfo.CurrentCulture, out mask) &&
										(byte.TryParse(uu[4], NumberStyles.HexNumber, CultureInfo.CurrentCulture, out equal) ||
			...
			t += s.Substring(from, s.Length - from);
			return t;
		}
	}
}
